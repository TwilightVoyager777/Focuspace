...
final class CenterRuleEngine {
    ...
    func compute(
    ...
    ) -> (dx: CGFloat, dy: CGFloat, strength: CGFloat, confidence: CGFloat) {
        ...
        CenterRuleEngine.ciContext.render(
            grayscale,
        ...
        for y in 0..<height {
            ...
            for x in 0..<width {
                ...
                sumW += weighted
                sumX += Float(x) * weighted
                sumY += Float(y) * weighted
            }
        }
        ...
        if sumW < verySmallThreshold {
            return (0, 0, 0, 0)
        }

        let cx = sumX / (sumW + 1e-6)
        let cy = sumY / (sumW + 1e-6)
        let cxn = CGFloat(cx) / CGFloat(width)
        let cyn = CGFloat(cy) / CGFloat(height)
        ...
        let strength = min(1, mag * 1.2)
        let confidence = min(1, CGFloat(sumW / energyThreshold))

        if confidence < 0.01 {
            return (0, 0, 0, 0)
        }

        return (dx, dy, strength, confidence)
    }
